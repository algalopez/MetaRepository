...
var $author$project$Game$Types$KeyPressed = function (a) {
	return {$: 'KeyPressed', a: a};
};
var $author$project$Game$Types$KeyReleased = function (a) {
	return {$: 'KeyReleased', a: a};
};
var $author$project$Game$Types$Tick = function (a) {
	return {$: 'Tick', a: a};
};
...
var $author$project$Main$startLoading = function (slingshot) {
	return _Utils_update(
		slingshot,
		{isLoading: true});
};
...
var $author$project$Game$Physics$updateProjectiles = F2(
	function (newTime, model) {
		var time = $elm$time$Time$posixToMillis(newTime);
		var gravity = (-9.8) * 4;
		var deltaTime = (!model.lastTime) ? 0 : ((time - model.lastTime) / 1000);
		var updateProjectile = function (proj) {
			if (!proj.active) {
				return proj;
			} else {
	...
	function (msg, model) {
		switch (msg.$) {
					...
					case 'ArrowLeft':
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									slingshot: A2($author$project$Main$moveSlingshot, model.slingshot, -5)
								}),
							$elm$core$Platform$Cmd$none);
					case 'ArrowRight':
						return _Utils_Tuple2(
					...
					case 'ArrowDown':
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									slingshot: $author$project$Main$startLoading(model.slingshot)
								}),
							$elm$core$Platform$Cmd$none);
					default:
			...
			case 'KeyReleased':
				var key = msg.a;
				if (key === 'ArrowDown') {
					if (model.slingshot.isLoading) {
						var releasedModel = _Utils_update(
							model,
							{
								slingshot: $author$project$Main$releaseSlingshot(model.slingshot)
							});
						return $author$project$Game$Physics$shootTomato(releasedModel);
					} else {
						return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
					}
				} else {
			...
			case 'Tick':
				var time = msg.a;
				var slingshot = model.slingshot;
				var updatedSlingshot = slingshot.isLoading ? _Utils_update(
					slingshot,
					{
						strength: A2($elm$core$Basics$min, 100, slingshot.strength + 2)
					}) : _Utils_update(
					slingshot,
					{
						strength: A2($elm$core$Basics$max, 0, slingshot.strength - 4)
					});
				var modelWithUpdatedSlingshot = _Utils_update(
					model,
					{slingshot: updatedSlingshot});
				return A2($author$project$Game$Physics$updateProjectiles, time, modelWithUpdatedSlingshot);
			case 'LoadSlingshot':
				return _Utils_Tuple2(
					_Utils_update(
						model,
			...
			case 'ReleaseSlingshot':
				return _Utils_Tuple2(
					...
					$elm$core$Platform$Cmd$none);
			default:
				return (!model.slingshot.isLoading) ? $author$project$Game$Physics$shootTomato(model) : _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
		}
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				'left',
				$elm$core$String$fromFloat(projectile.x) + '%'),
				A2(
				$elm$html$Html$Attributes$style,
				'bottom',
